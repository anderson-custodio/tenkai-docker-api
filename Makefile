IMAGE_REPO=softplan/tenkai-docker-api
TAG=$(TRAVIS_BRANCH)
...
#Build the binary
build: pre-build
	@echo "Building tenkai-docker-api"
	GOOS_VAL=$(shell go env GOOS) GOARCH_VAL=$(shell go env GOARCH) go build -v -a -installsuffix cgo -o ./build/tenkai-docker-api cmd/tenkai-docker/*.go

test:
	@echo "Testing tenkai-docker-api"
	cp tenkai-docker-api.yaml ~/
