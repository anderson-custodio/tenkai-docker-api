...
type VariableDAOInterface interface {
	...
	CreateVariableWithDefaultValue(variable model.Variable) (map[string]string, bool, error)
	...
	GetVarImageTagByEnvAndScope(envID int, scope string) (model.Variable, error)
}
...
func (dao VariableDAOImpl) CreateVariableWithDefaultValue(variable model.Variable) (map[string]string, bool, error) {

	auditValues := make(map[string]string)
	updated := false

	condition := model.Variable{
		EnvironmentID: variable.EnvironmentID,
		Scope:         variable.Scope,
		Name:          variable.Name,
	}
...
func (dao VariableDAOImpl) GetAllVariablesByEnvironmentAndScope(envID int, scope string) ([]model.Variable, error) {
	...
	if barIndex > -1 {
		if err := dao.Db.Where(&model.Variable{EnvironmentID: envID, Scope: scope}).Find(&variables).Error; err != nil {
			return nil, err
		}
	} else {
...
func (dao VariableDAOImpl) GetVarImageTagByEnvAndScope(envID int, scope string) (model.Variable, error) {
...
func (dao VariableDAOImpl) DeleteVariable(id int) error {
	return dao.Db.Unscoped().Delete(model.Variable{}, id).Error
}

//DeleteVariableByEnvironmentID - Delete environment
func (dao VariableDAOImpl) DeleteVariableByEnvironmentID(envID int) error {
	return dao.Db.Unscoped().Where(model.Variable{EnvironmentID: envID}).Delete(model.Variable{}).Error
}

//GetByID GetByID
func (dao VariableDAOImpl) GetByID(id uint) (*model.Variable, error) {
