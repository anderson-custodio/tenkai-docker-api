...
type HTTPClient interface {
	doRequest(url string, user string, password string, headers map[string]string) ([]byte, error)
}
...
func (h HTTPClientImpl) doRequest(url string, user string, password string, headers map[string]string) ([]byte, error) {
...
type DockerServiceInterface interface {
	...
	GetDockerVariables(repo *model.DockerRepo, imageName string, imageTag string) (
		*model.DockerVariablesResponse, error)
}
...
func (docker DockerService) GetDate(repo model.DockerRepo, imageName string, tag string) (*time.Time, error) {
	...
	body, err := docker.httpClient.doRequest(url, repo.Username, repo.Password, nil)
	if err != nil {
		return nil, err
	}
...
// GetDockerVariables return all variables from docker labels
func (docker DockerService) GetDockerVariables(repo *model.DockerRepo,
	imageName string, imageTag string) (*model.DockerVariablesResponse, error) {
	...
	if digest, err = docker.getDigest(repo, imageName, imageTag); err != nil {
		return nil, err
	}
	...
	url := fmt.Sprintf("https://%s/v2/%s/manifests/%s/blobs/%s", repo.Host,
		imageName, imageTag, digest)

	var dockerManifest []byte
	if dockerManifest, err = docker.httpClient.doRequest(url, repo.Username, repo.Password,
		headers); err != nil {

		return nil, err
	}

	var response model.DockerVariablesResponse
	if err = json.Unmarshal(dockerManifest, &response); err != nil {
		return nil, err
	}
...
func (docker DockerService) getDigest(repo *model.DockerRepo, imageName string,
	imageTag string) (string, error) {

	url := "https://" + repo.Host + "/v2/" + imageName + "/manifests/" + imageTag

	headers := make(map[string]string)
	headers["Accept"] = "application/vnd.docker.distribution.manifest.v2+json"

	body, err := docker.httpClient.doRequest(url, repo.Username, repo.Password, headers)
	if err != nil {
		return "", err
	}

	var dm model.DockerManifestResponse
	if e := json.Unmarshal(body, &dm); e != nil {
		return "", err
	}

	return dm.Config.Digest, nil
}
